...
let landingShaderPointer = { x: 0.5, y: 0.45 };
...
function resizeShaderCanvas(canvas, gl) {
...
function initLandingShader() {
  const canvas = document.getElementById("landing-shader-canvas");
  if (!canvas) return;
  ...
  const fsSource = `
    ...
    float lineField(vec2 p, float offset, float scale, float speed) {
      float wave = sin((p.x * scale) + (p.y * 1.7) + u_time * speed) * 0.08;
      float band = abs(p.y - offset - wave);
      return smoothstep(0.08, 0.0, band);
    }
    ...
    void main() {
      ...
      vec2 warp = vec2(
        ...
        fbm(p * 2.1 + vec2(-t * 0.6, t))
      );
      vec2 q = p + (warp - 0.5) * 0.34 + mouse * 0.05;

      float silk = fbm(q * 5.0 + vec2(u_time * 0.04, 0.0));
      float micro = fbm(q * 24.0 - vec2(0.0, u_time * 0.025));
      float scan = sin((uv.y + u_time * 0.012) * 900.0) * 0.018;

      float leftRail = lineField(q, -0.08, 4.1, 0.7);
      float rightRail = lineField(q, 0.17, 3.4, -0.55);
      float coreRail = smoothstep(0.055, 0.0, abs(q.y + sin(q.x * 2.4 + u_time * 0.18) * 0.035));

      float mouseWash = smoothstep(0.58, 0.0, length(q - mouse * 0.55));
      float vignette = smoothstep(0.95, 0.18, length(p));

      vec3 black = vec3(0.006, 0.004, 0.014);
      vec3 deepPurple = vec3(0.12, 0.07, 0.28);
      vec3 violet = vec3(0.45, 0.22, 0.88);
      vec3 lavender = vec3(0.82, 0.70, 1.0);
      vec3 amber = vec3(0.98, 0.54, 0.02);
      vec3 emerald = vec3(0.04, 0.73, 0.48);
      vec3 cyan = vec3(0.04, 0.63, 0.78);

      vec3 color = mix(black, deepPurple, 0.72 + uv.y * 0.35);
      color = mix(color, violet, smoothstep(0.12, 1.0, silk) * 0.5);
      color += lavender * pow(max(0.0, uv.y), 3.2) * 0.2;
      color += amber * leftRail * 0.26;
      color += emerald * rightRail * 0.25;
      color += cyan * coreRail * 0.18;
      color += violet * mouseWash * 0.18;
      color += vec3(micro * 0.08 + scan);
      color *= vignette;
  ...
  const drawQuad = createFullscreenQuad(gl, program);
  ...
  function render(time) {
    ...
    gl.uniform2f(mouseLoc, landingShaderPointer.x, landingShaderPointer.y);
    drawQuad();
